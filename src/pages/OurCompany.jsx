...
const OurCompany = ()=>{
  ...
  const handleMouseLeaveMI = ()=>{
   ...
   <>
      <HeroSection/>
      <div className={`w-full h-[60px]  justify-center items-center sticky top-0 hidden lg:flex`}>
      <StickyButton tag='TOYOTA EFFECT' width='w-[30%]' isVisible={toyotaIsHovered ? 'bg-white text-black': `bg-black text-white`} id='stickyButton' classname='stickyButton'/>
      <StickyButton  tag='WORKING AT TOYOTA'width='w-[40%]'border='border-l-1 border-r-1' borderColor='border-white' isVisible={wTIsHovered ? 'bg-white text-black': `bg-black text-white`}/>
      <StickyButton tag='MANUFACTURING HIGHLIGHTS' width='w-[30%]' isVisible={mIIsHovered ? 'bg-white text-black': `bg-black text-white`}/>
      </div>
